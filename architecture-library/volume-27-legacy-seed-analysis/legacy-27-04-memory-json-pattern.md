...
**Title:** Memory JSON Pattern
**Version:** v128 draft
**Status:** Draft
**Document ID:** MAL-V27-LEGACY-04

## Purpose
Document the safe legacy seed extracted from ai-business-os for the structured memory record pattern for agent context, customer facts, decisions, preferences, and operational state. This document preserves historical product and infrastructure intent while preventing unsafe migration of code, secrets, runtime data, or implementation-specific coupling.
...
- Legacy seed used structured JSON-like memory as a portable state pattern rather than opaque chat history.
- Mariam should preserve the pattern as a governed Memory Record with schema, owner, scope, expiry, provenance, confidence, and redaction fields.
- Raw historical memory content is not copied; only the structural pattern is retained.

## Extracted Pattern
- Treat `Memory JSON Pattern` as a canonical capability candidate, not as copied implementation.
...
| MAL-V27-LEGACY-04 | Safe v128 analysis of Memory JSON Pattern. |
...
| --- | --- | --- | --- |
| v128 draft | 2026-06-30 | Draft | Added safe legacy seed analysis for Memory JSON Pattern. |
